...
**11 production dependencies.** React, Zustand, FFmpeg WASM, mp4/webm muxers, HuggingFace Transformers, SoundTouch, WebGPU types. Everything else — the compositor, effects pipeline, keyframe system, export engine, audio mixer, text renderer, mask engine, scope renderers — is built from scratch. No UI component library, no animation framework, no CSS-in-JS.
